...
setting('coloring_window', 'Basic', ['Basic'], 'preference', 'hidden').setLabel(_('Coloring window type'), _('Select the interface used for USB coloring.'))
...
setting('steps_per_e', '0', float, 'machine', 'hidden').setLabel(_("E-Steps per 1mm for printer"), _("Amount of steps per mm filament extrusion. If set to 0 then this value is ignored and the value in your firmware is used."))
setting('color_steps_per_e', '', float, 'machine', 'hidden').setLabel(_("E-Steps per 1mm Spectrom"), _("Number of steps per mm filament extrusion, before the filament reaches the print head"))
setting('color_bodin_length', '', float, 'machine', 'hidden').setLabel(_("Bodin tube length"), _("The length, in mm, of the bodin tube connecting the spectrom device to the printer"))
